...
function openMobileMenu() {
  ...
  // Set focus on close button for accessibility after animation starts
  setTimeout(() => closeSidebar.focus(), 50);
}
...
function closeMobileMenu() {
  ...
  // Restore focus to menu toggle button
  menuToggle.focus();
}
...
// Keyboard Focus Trap within Mobile Sidebar
mobileSidebar.addEventListener("keydown", (e) => {
  if (!mobileSidebar.classList.contains("open")) return;
  
  const focusables = mobileSidebar.querySelectorAll('button, a');
...
// Clean up body scroll lock if viewport is resized past mobile breakpoint
window.addEventListener("resize", () => {
  if (window.innerWidth > 860 && mobileSidebar.classList.contains("open")) {
    closeMobileMenu();
  }
});
